...
session_start();
$id_login = $_SESSION['sessao_id'];
/* $consulta = $pdo->query("SELECT id_login FROM `login` WHERE usuario_login  LIKE '$extrato'");
$veri = $consulta->fetch(PDO::FETCH_ASSOC);
$id_login =  intval($veri['id_login']); */

$consulta2 = $pdo->query("SELECT id_usuario FROM `usuario` WHERE `login_id_login` = $id_login");
...
?>
    <form action="recebe_metas.php" method="POST">
